...
import type { MockupParams, ScreenshotParams } from "./types.js";
				...
				if (!args?.url || !args?.output_path) {
					return {
						...
						content: [
							{
								type: "text",
								text: "Missing required parameters: url and output_path",
							},
						],
					};
				}
				...
				if (!args?.url || !args?.output_dir) {
					return {
						...
						content: [
							{
								type: "text",
								text: "Missing required parameters: url and output_dir",
							},
						],
					};
				}
